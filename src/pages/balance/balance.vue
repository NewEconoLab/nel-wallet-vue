  ...
  <div class="container">
    <div class="title" style="padding-bottom:28px;">
      ...
      <div style="float:right">
        <span style="margin-right: 11px;color:#fff;">
          {{$t('balance.title2')}}：
          <span class="user-select-ok">{{currentAddress}}</span>
        </span>
        <button
          class="btn"
          data-toggle="modal"
          data-target="#selectAddr"
          v-if="chooseAddressarr &&chooseAddressarr.length>1"
        >{{$t('btn.switch')}}</button>
      </div>
    </div>
      ...
      <div>
        ...
        <div style="padding-left: 30px; padding-bottom: 30px;">
          ...
          <span style="vertical-align: super;margin-left: 10px;">
            ...
            <button
              class="btn btn-nel btn-disabled"
              disabled
              v-if="getGas==2"
            >{{$t('btn.gettingGas')}}</button>
            <v-hint>
              <div class="hint-img">
                <img src="../../../static/img/notice.png" alt style="width:20px;height:20px;">
              </div>
              <div class="hint-content hint-otherwidth">{{$t('balance.tips')}}</div>
            </v-hint>
          </span>
          <v-toast ref="toast"></v-toast>
        </div>
        <div style="padding-left: 30px; padding-bottom: 30px;">
          <span class="balance-type">NNC&nbsp;</span>
          <span class="balance-amount">{{nncBalance}}</span>
          <span style="vertical-align: super;margin-left: 10px;">
            <button class="btn btn-nel" v-if="getNNC==0" @click="getTestNNC">{{$t('btn.getNNC')}}</button>
            <button class="btn btn-nel btn-disabled" v-if="getNNC==1" disabled>{{$t('btn.getNNC')}}</button>
            <!-- <spinner-wrap :isbig="false" ></spinner-wrap> -->
            <button
              class="btn btn-nel btn-disabled"
              disabled
              v-if="getNNC==2"
            >{{$t('btn.gettingNNC')}}</button>
            <v-hint>
              <div class="hint-img">
                <img src="../../../static/img/notice.png" alt style="width:20px;height:20px;">
              </div>
              <div class="hint-content hint-otherwidth">{{$t('balance.tipsnnc')}}</div>
            </v-hint>
          </span>
        </div>
        <div class="claim" style="padding: 30px; padding-left: 2.3%;">
          ...
          <button
            class="btn btn-nel"
            v-if="neoasset.claim!='0'&&claimbtn"
            @click="toClaimGas"
          >{{$t('btn.claim')}}</button>
          <!-- <spinner-wrap :isbig="false"></spinner-wrap> -->
