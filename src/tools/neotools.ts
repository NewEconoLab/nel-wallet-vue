...
namespace tools
{
    ...
    export class neotools
    {
        ...
        {
            let result: tools.Result = new tools.Result();
            let login: tools.LoginInfo = new tools.LoginInfo();
            try
            {
         ...
         * nep2FromWif
         */
        public static nep2FromWif(wif: string, password: string): tools.Result
        {
            var prikey: Uint8Array;
            var pubkey: Uint8Array;
            var address: string;
            let res: tools.Result = new tools.Result();
            try
            {
         ...
         * nep2TOWif
         */
        public static async nep2ToWif(nep2: string, password: string): Promise<tools.Result>
        {
            var res = new tools.Result();
            let login: tools.LoginInfo = new tools.LoginInfo();
            let promise: Promise<tools.Result> = new Promise((resolve, reject) =>
            {
                let n: number = 16384;
                var r: number = 8;
                var p: number = 8
            ...
            try
            {
                ...
                if (wallet.accounts)
                {
                    for (let keyindex = 0; keyindex < wallet.accounts.length; keyindex++)
                    {
                        ...
                        try
                        {
                            let result: tools.Result = await neotools.getPriKeyfromAccount(wallet.scrypt, password, account);
        ...
        {
            let res = new tools.Result();
            let promise: Promise<tools.Result> =
